...
class StageConfig:
    ...
    num_goal_obs = 0  # goal features dim (0 for standard, 4 for track) / 目标特征维度
...
class TrackConfig(StageConfig):
    """
    Stage: navigation — learn track navigation with goal-directed locomotion.
    阶段：navigation —— 学习赛道导航与目标导向运动。

    Extends StageConfig with goal observation dimensions for track terrain.
    在 StageConfig 基础上增加 track 地形的目标观测维度。

    obs layout: proprio(45) + height_scan(256) + goal(4) = 305
    critic obs: critic_proprio(60) + height_scan(256) + goal(4) = 320
    """
    ...
    num_goal_obs = 4
    # policy obs = proprio(45) + scan(256) + goal(4) = 305
    # critic obs = critic_proprio(60) + scan(256) + goal(4) = 320
    num_critic_observations = 320

    # Last-minute Track transfer settings.
    # Keep the trained 301-dim Standard gait intact and inject navigation through
    # the existing velocity-command channel instead of replacing the model.
    ...
    nav_cmd_scanner_max_m = 2.5
    nav_cmd_min_valid_ray_ratio = 0.05
    nav_cmd_front_block_m = 0.55
    nav_cmd_deadend_m = 0.75
    nav_cmd_avoid_yaw = 0.75

    # Conservative fine-tuning from a good Standard checkpoint.
    lr = 1e-4
...
class Config:
    ...
    #   LocomotionConfig → standard 地形基础运动训练
    #   TrackConfig      → track 地形导航训练
    # 通过修改 CURRENT 切换阶段：
    #   LocomotionConfig → standard 地形基础运动训练
    #   TrackConfig      → track 地形导航训练
    CURRENT = TrackConfig
